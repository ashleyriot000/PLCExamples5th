...
public class MagneticSensor : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        //게임오브젝트 안에 감지할 콜라이더가 없으면 콜라이더를 추가하고
        if (c == null)
    ...
    {
        //감지 가능한 레이어들 중에서 트리거된 게임오브젝트가 레이어에 포함되지 않는다는 뜻.
        if ((detectableLayer.value & 1 << other.gameObject.layer) == 0)
            return;

        //감지 가능한 태그가 입력되어 있고, 트리거된 게임오브젝트의 태그가 감지가능한 태그와 다를 경우 무시해라.
        if (!string.IsNullOrEmpty(detectableTag) && other.gameObject.tag != detectableTag)
            return;

        //감지 가능한 이름이 입력되어 있고, 트리거된 게임오브젝트의 이름 안에 감지가능한 이름이 포함되지 않으면 무시해라.
